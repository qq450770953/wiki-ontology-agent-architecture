...
> 版本：v1.2（2026-09-18）
> 状态：MVP 规划 + 外部方案借鉴映射（对应 architecture.md v1.8）
> 读者对象：企业知识库 / AI Agent 平台的技术负责人、架构师（本仓库 owner 健哥）
> 目的：回答「参考乌圆AI 自研 Ontology Engine 的 MVP 方案，本架构如何以最小代价跑通第一个闭环」
> 相关：`docs/architecture.md`（v1.8）§4.3 六步管线 / §4.4 动态自主度调节 / §9 安全治理 / §11 落地路线图 / §11.1 参考实现映射
...
## 2. 借鉴映射：4 个采纳点（已落 architecture.md，v1.6 落地、现行 v1.8）
...
| 阶段 | 内容 | 产出 |
|---|---|---|
| **W1-2 语义底座** | **数据盘点先行（v1.2，M3）**：盘点 device-station-fault 存量故障码数据的质量/覆盖率/更新频率，按"盘点→知识识别→语义映射→构建→验证"路线裁剪 schema，防"过设计"；随后 YAML 定义核心 Object/Link/Action 类型；audit 表升级为 6 字段 append-only（借鉴点 1/4 落地） | 数据盘点报告 + Schema 注册表最小版 + 合规审计表 |
| **W3-4 执行链路** | 六步管线 × Action 7 步融合：治理三道门（参数/权限/规则）→ 内部事务 → 外部 Saga 补偿（借鉴点 2）→ 审计；物化加 user edits win（借鉴点 3） | 可执行、可审计、可补偿的 Action 链路 |
| **W5 闭环沉淀** | 上下文物化 → LLM 汇总（读 ContextPack）→ 人工审批闸口 → 过审流程注册 Wiki 注册表（条目含 `applicable_domain` 适用域 + 分类标签，对应 architecture v1.8）+ 命中直调（含超域降级检查） | 知识闭环可运转 + 注册页资产字段就位 |
| **W6 试运行** | 1 个真实场景跑 7 天 + 业务验收 | 闭环验证 |

### 4.1 试点选择：场景四维评估矩阵（v1.2，M1）

参考汽车研发白皮书 §8.2 的场景研判方法，用**业务价值 / 技术可行性 / 数据基础 / 可复制性**四维打分（1-5 分）论证试点选择——既论证"为什么是设备售后"，也给后续场景（MES 经营数据、线束设备等）提供可复用的排序工具：

| 维度 | 设备售后"故障码→处置动作" | MES 经营数据查询 | 说明 |
|---|---|---|---|
| 业务价值 | 5（处置时效直接关系停机损失） | 3（查询提效，价值偏分析侧） | 停机成本可量化 |
| 技术可行性 | 4（Action 链路含工单回写，治理链路完整覆盖） | 4（以只读查询为主，链路更短） | 都在 Action 7 步能力内 |
| 数据基础 | 4（device-station-fault 层级结构已建成） | 3（依赖 MES 接口与口径梳理） | M3 数据盘点在 W1-2 前置验证此项 |
| 可复制性 | 5（device-station-fault 模式可复制到其他设备线） | 4（查询范式通用） | 白皮书强调"优先可复制场景" |
| **合计** | **18** | **14** | 试点 = 设备售后 |
...
✅ 注册表条目含 `applicable_domain`（对象类型+状态范围+参数边界），超域执行自动降级 supervised 并留痕（对应 architecture v1.8 适用域声明）
✅ 动态降级演练通过：注入连续低置信度结果后，自主度自动收紧一档并告警，恢复后逐档放宽（对应 architecture v1.8 动态自主度调节）
...
- 本仓库 `docs/architecture.md`（v1.8）§4.3 / §4.4 / §9 / §10 / §11 / §11.1（借鉴点落位）
- 《智能体赋能汽车研发设计白皮书——从工具辅助到智能原生》（2026，中国汽研牵头、一汽/长安/理想等 15 家企业联合发布）——v1.2 新增：§8.2 场景四维评估矩阵（本文 §4.1）、§7.1.1 数据治理先行构建路线（W1-2 数据盘点）、§7.6.3 技能资产治理（注册页标签体系与使用评价回流）
