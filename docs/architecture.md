...
> - **适用域声明（H2）**：§4.5 / §6.5 固化流程注册表条目强制含 `applicable_domain`（对象类型 + 状态范围 + 参数边界），命中执行前做超域检查，超域自动降级 supervised；§9 补白皮书八项可信原则对照。
> - **技能资产治理（M2）**：§6.5 注册页补分类标签体系与使用评价回流（执行次数/成功率/耗时/反馈），注册表从"静态目录"变"活资产"。
> - **场景四维评估矩阵（M1）+ 数据治理先行（M3）**：mvp-plan 升 v1.2——业务价值/技术可行性/数据基础/可复制性四维研判论证试点选择；W1-2 前置数据盘点防"过设计"。
> - **L 项**：§7 补 token 计量（"智能量"）展望、§4.7 补 A/B 对照测试方法论、§12 补白皮书参考（其"通用/垂域/机理三层模型分工"印证"确定性节点不调用 LLM"）。
>
...
**动态自主度调节（运行时降级，v1.8）**：上表的自主度是**部署时基线**，不是一成不变的静态配置——运行期必须按实际表现动态收紧/放宽（参考汽车研发白皮书 §4.4：智能体连续输出低置信度结果或检测到异常模式时，系统自动切换为人工运行模式）：

- **降级触发**：同一 Workflow / 场景连续 N 次低置信度结果、四层评估指标越回退阈值（§4.7）、异常告警（工具失败率 / 越权尝试）超限 → 自动将自主度收紧一档（auto→supervised→gated）并告警值守。
- **恢复条件**：连续 M 次通过检查点 + Golden 回归通过后，才允许逐档放宽；降级 / 恢复事件全部留痕审计。
- **命中路径超域降级**：固化流程注册表条目带适用域声明（§4.5 / §6.5），执行前超域检查不通过 → 本次执行自动降级 supervised。
- **与评估打通**：§4.7 评估不只告警，还**驱动自主度调整**——形成"评估 → 调节 → 再评估"闭环。
...
3. **更新 Wiki 连接**：审批通过后，把新 Workflow 注册进 Wiki 的**固化流程注册表**（记录问题范式 → Workflow ID → 版本 → 生效时间 → 负责人 → **适用域 applicable_domain**），旧版本标记"已替代"而不是删除。**审批未通过则不产生任何 Wiki 连接。** 适用域声明该流程可自动执行的对象类型、状态范围与参数边界——命中执行前做超域检查，超出适用域自动降级 supervised（见 §4.4 动态自主度调节）。
...
评估方法论补充（v1.8）：对"智能体辅助 vs 传统人工模式"做 **A/B 对照测试**，量化对比吞吐、返工率、正确率，作为自主度放宽（§4.4 动态调节）与试点推广决策（mvp-plan 四维矩阵）的依据；评估结果同时驱动运行时自主度收紧/放宽——评估不只告警，还要闭环到 §4.4。
...
每页至少包含：唯一 ID、类型（实体/概念/流程/模板）、标题、正文、来源、版本、生效时间、状态（active / superseded）、负责人、关联页面。其中**"流程"页 = 固化流程注册页**，额外保存 `workflow_id`（指向 LangGraph Workflow 定义）、审批记录与 **`applicable_domain`（对象类型 + 状态范围 + 参数边界，v1.8 适用域声明）**，入口路由层凭它命中并加载对应图；命中执行前做超域检查（§4.4）。

**注册页资产字段（v1.8 技能资产治理，参考汽车研发白皮书 §7.6.3）**：

- **分类标签体系**：领域标签（area）、风险档位、触发范式标签——支撑检索、统计与场景聚类。
- **使用评价回流**：执行次数、成功率、P50/P95 耗时、最近执行时间、用户反馈摘要——注册表从"静态目录"变"活资产"，并为 §4.4 动态自主度调节提供数据源。
...
- **token 计量展望（v1.8）**：随智能体规模化，成本口径可从 token 延伸为按任务/场景计量的"智能量"——把每次固化命中节省的 token 折算为复利收益，作为 §4.7 A/B 对照与场景推广排序（mvp-plan 四维矩阵）的量化指标（参考汽车研发白皮书 §13.3）。
...
- **分级授权矩阵**：按复杂度分档决定自主度与监控频率（见 §4.4 表 + 动态自主度调节）；运行期以实时异常告警 + 检查点 + 事后审查为主，**避免逐级审批疲劳化**。
- **适用域声明与超域降级（v1.8）**：固化流程注册表条目强制含 `applicable_domain`（对象类型 + 状态范围 + 参数边界，§4.5/§6.5）；命中执行前做超域检查，超出适用域自动降级 supervised 并告警。白皮书八项可信原则在本架构的对应：上下文前置 → ContextPack（§3.2-5）；可追溯 → 审计 6 字段（本节）；人在回路 → human_approval / 固化审批（§4.5）；规则与模型分工 → SQL 模板白名单（本节）；签核 → 分级审批；白名单 → Ontology 实体/参数校验（§4.3）；失败安全 → Saga 补偿 + MANUAL_REVIEW（§4.3-4）。
